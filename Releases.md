...
- **Dashboard ESC Temperature Gauge Arc Coloring (`widgets/dashboard/objects/gauge.lua`)** (fixes #132):
  ...
  - Added explicit temperature source detection (`isTempSource`) with inverted threshold coloring: low/cool = green, high/hot = orange/red.
  - Temperature thresholds resolve from box properties (`warntemp`/`alerttemp`), theme configuration (`esctemp_warn`/`esctemp_alert`), or sensible defaults (90 °C warn, 105 °C alert, with alert clamped to gauge maximum to remain reachable).
  - Preserved valid cold / freezing temperatures ($\le 0$ °C) in OK green and properly distinguish missing telemetry (`curHasValue == false`) for arc background coloring.
  - Passed temperature and scale flags directly from `renderArc` to avoid redundant lookups and eliminated unit/source mismatches under Fahrenheit mode.
