...
- **Dashboard Widget Preferences Sync, Stats Precedence & Governor Optimization (`widgets/dashboard`, `theme/page.lua`)**:
  - **Per-Model Preferences Change Detection & Memoization**: Cloned `session.modelPreferences` and its `dashboard` section table on save in `theme/page.lua` so table identity changes upon save, allowing the dashboard widget background loop to immediately detect configuration changes. Keyed `themePathMemo` in `runtime.lua` on the content signature (`modelSig`) rather than table reference pointer equality, preventing stale theme resolution and eliminating redundant re-resolution logging.
  - **Stats Box Flight Precedence & BEC Voltage Correction**: Corrected stat value precedence in `widgets/dashboard/objects/text/stats.lua` so current flight extremes (`currentFlight...`) take precedence over previous flight aggregates (`lastFlight...`) while in flight, falling back to previous flight values once disarmed. Resolved a copy-paste error in the `bec_voltage` branch where main battery pack voltage (`lastMinVoltage`) was displayed instead of BEC voltage, and implemented in-flight minimum BEC voltage tracking (`currentFlightMinBecVoltage` / `lastMinBecVoltage`) in `runtime.lua`.
