...
- **AM32 ESC preserves timing advance wire encoding on save (`tasks/msp/api/esc_parameters_am32.lua`) (fixes #282)**:
  - Saving the AM32 page always encoded the timing advance byte using the new format (`10 + 8 * value`), which corrupted legacy-encoded ESC configurations (raw bytes 0..3 were rewritten as 10, 18, 26, or 34).
  - The parse now tracks whether the timing advance was received in the legacy (0..3) or new (10..42) wire encoding and preserves this flag in the parsed data, allowing `buildWritePayload` to encode and write back the timing advance matching the ESC firmware's expected encoding.
